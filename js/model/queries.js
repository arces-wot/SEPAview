function queryDashboardURL(foi) {
	...
	sepa.query(prefixes + " " + query, jsap).then((data) => {
		if (data.results.bindings.length == 0) onQueryDashboardURL(undefined,foi);
		else for (binding of data.results.bindings) {
				if (binding.dashboard != undefined) onQueryDashboardURL(binding.dashboard.value,foi);
				else onQueryDashboardURL(undefined,foi);
			}
	});
}
